...
🔗 Aplicação em produção: chatbotsispetro.com.br
...
Extração — script Python que coleta as páginas do espaço técnico do Sispetro no Atlassian Confluence via API, salvando o conteúdo bruto em HTML.
Pré-processamento — limpeza dos arquivos HTML com Beautiful Soup, convertendo o conteúdo para texto puro (.txt, UTF-8), preservando título e URL de origem de cada página.
Fragmentação (chunking) — segmentação dos textos com o RecursiveCharacterTextSplitter do LangChain.
Vetorização (embeddings) — geração de vetores semânticos com o modelo text-embedding-3-small da OpenAI.
Indexação — armazenamento dos vetores no Pinecone, banco de dados vetorial em nuvem.
...
bash   git clone https://github.com/vanrpontes/tcc-ufsc-contabilidade-rag.git
...
(Opcional) Execute o pipeline de extração e ingestão da base de conhecimento, caso deseje popular o índice vetorial com seus próprios dados:


bash   python backend/scripts/extract.py
...
Para dúvidas sobre o projeto ou sobre o TCC, entre em contato: vrpontes@outlook.com
